...
import { AppUser, RefreshToken, Role } from '../schema';
...
export class AuthService {
  ...
  async signup(data: Signup): Promise<void> {
    ...
    // Validate role if provided, otherwise default to Repairman
    const userRole = data.role || Role.Repairman;
    
    if (!Object.values(Role).includes(userRole)) {
      throw new Conflict('Neteisingas vaidmuo (role)', 400);
    }
  ...
  async login(data: Login): Promise<[string, string]> {
    if (!data.username) {
      throw new Conflict('Nepaduotas prisijungimo vardas', 409);
    }

    const user = await this.authRepository.getUserByUsername(data.username);

    if (_.isEmpty(user)) {
      throw new InvalidCredentials('Vartotojas su tokiu prisijungimo vardu neegzistuoja', 400);
    }

    const match = await bcrypt.compare(data.password, user.password);

    if (!match) {
      throw new InvalidCredentials('Neteisingas slaptažodis', 400);
    }
  ...
  async refresh(refreshToken: string) {
    ...
    if (_.isEmpty(token)) {
      throw new Conflict('Refresh tokenas neegzistuoja', 409);
    }
